...
exports.handler = async (event) => {
  ...
  try {
    ...
    const currentReleasedAmount = Number(order.fundsReleasedAmount || 0);
    ...
    if (milestone === "PICKED_UP") {
      ...
      if (
        ...
        console.log(`Pickup milestone already processed for order ${orderID}`);
      ...
      const firstReleaseAmount = Number((earnings / 2).toFixed(2));
      ...
      const newPendingBalance = Number(
        (pendingBalance - firstReleaseAmount).toFixed(2),
      );

      const newAvailableBalance = Number(
        (availableBalance + firstReleaseAmount).toFixed(2),
      );
      ...
      const walletUpdate = await updateWallet({
        wallet,
    ...
    if (milestone === "DELIVERED") {
      ...
      const remainingAmount = Number(
        (earnings - currentReleasedAmount).toFixed(2),
      );
      ...
      if (order.fundsStatus === "RELEASED" || remainingAmount <= 0.01) {
        console.log(
      ...
      const newPendingBalance = Number(
        (pendingBalance - remainingAmount).toFixed(2),
      );

      const newAvailableBalance = Number(
        (availableBalance + remainingAmount).toFixed(2),
      );
...
async function getCourierWallet(courierID) {
  ...
  return response?.data?.listWallets?.items?.[0] || null;
}
...
async function updateWallet({ wallet, availableBalance, pendingBalance }) {
  ...
  if (response.errors) {
    throw new Error(
      `Failed to update courier wallet: ${JSON.stringify(response.errors)}`,
    );
  }

  return response?.data?.updateWallet || null;
}
...
async function updateOrder({
  ...
  return response?.data?.updateOrder || null;
}
